...
/*   Updated: 2026/03/12 16:30:31 by rfleritt         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
t_color	parse_color(char *color)
{
	...
	while (split[++i])
		free(split[i]);
	if (rgb.r < 0.0f || rgb.r > 255.0f || rgb.g < 0.0f || rgb.g > 255.0f
		|| rgb.b < 0.0f || rgb.b > 255.0f)
		return (free(split), exit_msg_error("Color admit numbers 0/255"), rgb);
